...
- **Regla para IA al escribir manifiestos: usa SIEMPRE rutas relativas completas** (`videos/2/clip1.MOV`, no `clip1.MOV`). Con ruta, tanto el CLI como la carpeta conectada de la web resuelven el archivo EXACTO; sin ruta, ambos buscan por nombre y toman el primero que encuentren — con nombres repetidos entre carpetas (`clip1.MOV`, `untitled.wav` existen en varios proyectos del autor) cargarás el archivo equivocado. La búsqueda por nombre queda solo como respaldo.
...
## 9. Herramientas opcionales del pipeline
